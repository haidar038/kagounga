...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      // Create a map of user_id -> isAdmin
      ...
      // For now, we'll just show users who have roles assigned
      const usersWithRoles: UserWithRole[] = userIds.map((userId) => ({
        id: userId,
        email: `User ${userId.substring(0, 8)}...`, // We can't access auth.users directly
        created_at: new Date().toISOString(),
        isAdmin: adminMap.get(userId) || false,
      }));
  ...
  const addAdminByEmail = async () => {
    ...
    try {
      ...
      toast.info(
        "To add an admin, the user must first sign up. Then use their User ID from the database."
      );
      setDialogOpen(false);
      setNewAdminEmail("");
    } finally {
        ...
        {isLoading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-sm text-muted mt-2">
              Users will appear here once they sign up and are assigned roles.
            </p>
          </div>
        ) : (
          <Table>
