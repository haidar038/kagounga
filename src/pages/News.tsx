      ...
      <section className="bg-secondary/30 py-16 sm:py-20">
        <div className="container-page text-center">
          ...
          <p className="mt-4 text-lg text-muted max-w-2xl mx-auto">
            Stay updated with the latest news, events, and stories from Kagōunga.
          </p>
        </div>
      </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post) => (
                <Link
                  key={post.id}
                  to={`/news/${post.slug}`}
                  className="group rounded-2xl bg-card border border-border overflow-hidden hover:border-primary-border hover:shadow-soft transition-all"
                >
                  {/* Image */}
                  ...
                  <div className="p-6">
                    ...
                    <h2 className="font-heading text-xl font-semibold group-hover:text-accent transition-colors line-clamp-2">
                      {post.title}
                    </h2>
                    {post.excerpt && (
