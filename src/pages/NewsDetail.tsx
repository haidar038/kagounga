...
const NewsDetail = () => {
    ...
    const {
        ...
        queryFn: async () => {
            const { data, error } = await supabase.from("news_posts").select("*").eq("slug", slug).eq("is_published", true).maybeSingle();
    ...
    return (
        <Layout>
            ...
            <article className="py-16 sm:py-20">
                <div className="container-page max-w-3xl">
                    {/* Back link */}
                    <Link to="/news" className="inline-flex items-center gap-2 text-sm text-muted hover:text-foreground mb-8">
                    ...
                    <header className="mb-8">
                        <h1 className="font-heading text-3xl font-bold sm:text-4xl lg:text-5xl leading-tight">{post.title}</h1>
                    ...
                    {/* Excerpt */}
                    {/* {post.excerpt && <p className="text-xl text-muted leading-relaxed mb-8 font-medium">{post.excerpt}</p>} */}
