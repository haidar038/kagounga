...
export function MissionCard() {
    ...
    const { data: mission, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const { data, error } = await supabase.from("mission_content").select("*").limit(1).maybeSingle();
                        ...
                        <div className="order-1 lg:order-2">
                            {mission.image_url && (
                                <div className="relative rounded-2xl overflow-hidden aspect-[4/3] lg:aspect-square">
                                    <img src={mission.image_url} alt="Kagōunga mission" className="w-full h-full object-cover" loading="lazy" />
