            ...
            <div className="container-page">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {points?.map((point, index) => {
                        ...
                        return (
                            <div key={point.id} className="group p-6 rounded-2xl bg-card border border-border hover:border-primary-border hover:shadow-soft transition-all duration-300" style={{ animationDelay: `${index * 100}ms` }}>
                                <div className="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-xl bg-primary/20 text-lime-600 group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
                                    <IconComponent className="h-6 w-6" />
                                </div>
                                <h3 className="font-heading text-lg font-semibold mb-2">{point.heading}</h3>
